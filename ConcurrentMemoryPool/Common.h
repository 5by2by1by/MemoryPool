...
const size_t NLISTS = 184;                   //由对齐规则计算得来，总共184个内存对象，由FreeList类管理，FreeList实例化就是一个内存对象，有8B、16B、128B、1KB的对象，相同的内存对象用单链表形式串联起来
const size_t PAGE_SHIFT = 12;          // 规定一页大小 4 << 12 = 4k
const size_t NPAGES = 129;                 // 页表长度

/* 我理解的这个函数就是为了 在当前对象中保存下一个内存对象的地址，为了有效回收*/
...
class Freelist{
	private:
		// _list指向第一个对象，但里面存着第二个对象的地址。
		void* _list = nullptr; // 给上缺省值
		size_t _size = 0;  // 记录有多少个对象，自由链表下挂载对象的数目；
		//水位线，就是当自由链表下面的内存块对象的个数大于或者等于这个数时，才进行释放
		size_t _maxsize = 5;   // 自由链表下挂对象的最大个数
	...
	public:
		...
		void PushRange(void* start, void* end, size_t n){    // 一下子压入n个内存块对象
			NEXT_OBJ(end) = _list;
			_list = start;
			_size += n;
		}
...
public:
	...
	{
		size_t alignnum = 1 << align;  //库函数实现的方法
		return ((size + alignnum - 1) >> align) - 1;
	}

	/*    index:              memoryBlocks              意义
			0~15				前16个8字节              想要128字节，那就去第15个index申请内存
...
public:
	// 控制在12%左右的内碎片浪费                                                对齐的字节数越少，浪费的越少，内碎片越少。
	// 比如我想要申请一个7B内存大小，那么只能申请到8个字节，内碎片是1个字节。
	// [1,128]				8byte对齐 freelist[0,16)                                     最常用的对齐方式
	// [129,1024]			16byte对齐 freelist[16,72)
	// [1025,8*1024]		128byte对齐 freelist[72,128)
	// [8*1024+1,64*1024]	1024byte对齐 freelist[128,184)
	// 总共分了四个区间，分别是8、16、128、1K字节对齐！（定长分配内存）
